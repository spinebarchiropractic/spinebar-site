        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-12">
          <div className="grid gap-16 lg:grid-cols-[1fr_300px] lg:items-start">
            ...
            <article>
              ...
              <FadeIn className="prose-custom space-y-6" delay={0.08}>
                {post.body.map((block, i) => {
                  if (block.type === "heading") {
                    return (
                      <h2
                        key={i}
                        className="mt-10 font-serif text-3xl leading-tight text-green"
                      >
                        {block.content}
                      </h2>
                    );
              ...
              <FadeIn delay={0.15} direction="left">
                ...
                {related.length > 0 && (
                  <div className="rounded-2xl border border-cream-border bg-cream-dark p-6">
                    ...
                    <ul className="space-y-4">
                      {related.map((r) => (
                        <li key={r.slug}>
                          <Link
                            href={`/blog/${r.slug}`}
                            className="group block rounded-xl p-3 transition-colors hover:bg-cream"
                          >
                            <p className="mb-1 text-xs uppercase tracking-[0.15em] text-gold">
                              {r.category}
                            </p>
                            <p className="text-sm font-medium leading-snug text-green group-hover:text-green-dark">
                              {r.title}
                            </p>
                          </Link>
                        </li>
                    ...
                    <Link
                      href="/blog"
                      className="mt-4 block text-sm font-medium text-gold transition-colors hover:text-green"
                    >
                      All articles →
                    </Link>
                  </div>
